...
# specify prior family during initialization (e.g. binary, point-exponential)
...
alt.flash <- function(fit.init,
  ...
  for (i.iter in 1:maxiter){
    ...
    # update q
    obj.temp.l <- SOL.Q(Y, D, A.l, A.f, B.f, tau, g.l, g.l.pf, sol.reg.max.iter=sol.reg.max.iter)
    ...
    obj.temp.f <- SOL.Q(t(Y), D, A.f, A.l, B.l, tau, g.f, g.f.pf, sol.reg.max.iter=sol.reg.max.iter)
    A.f <- obj.temp.f$A.l;  B.f <- obj.temp.f$B.l;  C.f <- obj.temp.f$C.l;  KL.f <- obj.temp.f$KL
    ...
    # compute elbo
    elbo <- ELBO(Y=Y, trYTY=trYTY, tau=tau,
                 A.l=A.l%*%sqrt(D), A.f=A.f%*%sqrt(D),
                 B.l=sqrt(D)%*%B.l%*%sqrt(D),
                 B.f=sqrt(D)%*%B.f%*%sqrt(D), KL.l, KL.f)
    if(print.elbo==TRUE){print(paste0(Sys.time(), "; iter=", i.iter, "; elbo=", elbo))}

    # update scale matrix D
    if(g.l.pf=='binary' & g.f.pf=='binary'){
      D <- D.qp(Y, A.l=A.l, B.l=B.l, A.f=A.f, B.f=B.f, scale.min=1e-10, print.scale=FALSE)

      # check NULL factor
      k.null <- diag(D)<1e-9 # 10x  scale.min
      if(sum(k.null)>0){
        print(paste0('drop ', sum(k.null), ' factor(s); ', sum(!k.null), ' factor(s) remaining'))
    ...
    #plotLF(A.l, A.f, 'L and F')

    # update tau
    tau <- SOL.TAU(trYTY=trYTY, Y=Y,
                   A.l=A.l%*%sqrt(D), A.f=A.f%*%sqrt(D),
                   B.l=sqrt(D)%*%B.l%*%sqrt(D), B.f=sqrt(D)%*%B.f%*%sqrt(D))


    # update g
    if(g.common==TRUE){
      g.lf <- SOL.G.COMMON(g.l, g.l.pf, Y, A.l, A.f, B.l, B.f, tau)
      g.l <- g.lf; g.f <- g.lf
    }
    if(g.common==FALSE){
      g.l <- SOL.G(g.l, g.l.pf,    Y, A.l, A.f, B.f, tau)
      g.f <- SOL.G(g.f, g.f.pf, t(Y), A.f, A.l, B.l, tau)
    }
  ...
  scales <- sqrt(diag(B.l) * diag(B.f))*diag(D)
...
# given 'L' and 'Y', create 'init.fit' object
INIT.L <- function(L, Y, g.pf, D=NULL){

  n <- nrow(Y); p <- ncol(Y); K <- ncol(L)
  A.l <- L; A.f <- L
  if(is.null(D)){D <- diag(x=rep(1,K), nrow=K)}

  g.lf <- list()
  if(g.pf=='point_exponential'){
    for (k in 1:K){
      g.lf[[k]] <- gammamix(pi=c(mean(A.l[,k]==0), mean(A.l[,k]!=0)),
  ...
  if(g.pf=='binary'){
    for (k in 1:K){
      b <- 1
      pi.b <- mean(A.l[,k]==1)
      g.lf[[k]] <- ashr::normalmix(pi=c(1-pi.b, pi.b), mean=c(0, b), sd=c(0, 0))
      }
  }
  g.l <- g.lf; g.f <- g.lf
...
G.PF <- function(g){
  ...
  if( all(sapply(g, function(x) class(x)=="normalmix")) &
      all(sapply(g, function(x) x$sd==0)) &
      all(sapply(g, function(x) x$mean[1]==0)) &
      all(sapply(g, function(x) x$mean[2]==1)) &
      all(sapply(g, function(x) length(x$mean)==2))
      ) {g.pf <- 'binary'}
...
SOL.Q <- function(Y, D, A.l, A.f, B.f, tau, g.l, g.l.pf, sol.reg.max.iter){
  ...
  for (it in 1:sol.reg.max.iter){
    for (k in 1:K){
      ...
      if(g.l.pf=='binary'){
        g.l.k.pi <- g.l[[k]]$pi[2]
        g.l.b <- g.l[[k]]$mean[2]
        chi.l.k.i <- exp(tau*(Y %*% A.f[,k] - A.l[,-k]%*% B.f[-k,k])*g.l.b - tau/2*B.f[k,k]*g.l.b^2)

        w <- 1 - (1-g.l.k.pi)/((1-g.l.k.pi) + g.l.k.pi *chi.l.k.i)
        A.l[,k]  <- w*g.l.b
        C.l[,k] <- w*g.l.b^2

        KL[k] <- sum(w*log(g.l.k.pi) - sapply(w, xlogx) + (1-w)*log(1-g.l.k.pi) - sapply(1-w, xlogx))
        if(g.l.k.pi==1){KL[k] <- 0}
      }else{
...
# update g_l (or g_f)
SOL.G <- function(g.l, g.l.pf, Y, A.l, A.f, B.f, tau){

  K <- ncol(A.l)

  for (k in 1:K){

    if(g.l.pf=='binary'){ g.l[[k]]$pi[2] <- mean(A.l[,k])
    }else{
      x <- c(Y%*%A.f[,k] - A.l[,-k, drop=FALSE] %*% B.f[k,-k])/B.f[k,k]
      s <- rep(1/sqrt(tau*B.f[k,k]), length(x))

      obj.temp <- ebnm(x=x, s=s, prior_family=g.l.pf, fix_g=FALSE, output='fitted_g')
...
SOL.G.COMMON <- function(g.l, g.l.pf, Y, A.l, A.f, B.l, B.f, tau){

  K <- ncol(A.l)

  for (k in 1:K){

    if(g.l.pf=='binary'){ g.l[[k]]$pi[2] <- mean(c(A.l[,k], A.f[,k]))
    }else{
      ...
      obj.temp <- ebnm(x=c(x.l, x.f), s=c(s.l, s.f), prior_family=g.l.pf, fix_g=FALSE, output='fitted_g')
      g.l[[k]] <- obj.temp$fitted_g
    }
  }
