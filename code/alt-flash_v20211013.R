...
# changes (v. 20211010)
# - SOL.REG mat.iter set as 0
# - terminate if elbo decreases
...
alt.flash <- function(flashier.fit=NULL, # initialize with flashier obj
  ...
  if(is.null(flashier.fit)){ obj.temp <- INIT.rand(Y, g.l, g.f, seed, print.elbo=print.elbo, sol.reg.max.iter=sol.reg.max.iter) }
...
INIT.rand <- function(Y, g.l, g.f, elbo.tol=1, max.update=30, min.update=10, seed=NULL,
  ...
  for (i.iter in 1:max.update){
    ...
    tau <- SOL.TAU(trYTY=trYTY, Y, A.l, A.f, B.l, B.f)
    elbo <- ELBO(Y, trYTY, tau, A.l, A.f, B.l, B.f, KL.l, KL.f)
    if(print.elbo==TRUE){print(paste0(Sys.time(), "; (init) iter=", i.iter, "; elbo=", elbo))}

    if(i.iter > min.update){if(abs(elbo.old-elbo) < elbo.tol) break}
    elbo.old <- elbo
  }
...
SOL.REG <- function(y, b, A.f, B.f, tau, g.l, g.l.pf, max.iter, tol=1e-10){
...
SOL.Q <- function(Y, A.l, A.f, B.f, tau, g.l, g.l.pf, sol.reg.max.iter){
  ...
  for (i in 1:n){
    temp.obj <- SOL.REG(y=Y[i,], b=A.l[i,], A.f, B.f, tau, g.l, g.l.pf, max.iter=sol.reg.max.iter)
    A.l[i,] <- temp.obj$Et
    C.l[i,] <- temp.obj$Et2
