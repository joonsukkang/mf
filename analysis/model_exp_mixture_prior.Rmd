...
demo.update.B <- function(mat.X, Elsq, eta.grid, pi.grid, niter.updates = 10){
    ...
    pi.ik <- matrix(0, nrow=N, ncol=K)
    for (k in 1:K){
      pi.ik[,k] <- pi.grid[k]/sqrt(2*eta.grid[k])*exp(-E.abs.l/sqrt(eta.grid[k]/2))
    }    
    for (i in 1:N){ # normalize to sum to 1 for each row  
      pi.ik[i,] <- pi.ik[i,]/sum(pi.ik[i,])   
    }
    
    Ew.inv <- c(sqrt(2/Elsq) * (pi.ik %*% (1/sqrt(eta.grid))))
    
    out.list = list()
    out.list$Ew.inv = Ew.inv
    out.list$pi.ik = pi.ik
