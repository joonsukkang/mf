...
INIT.L.NonNeg <- function(Y, verbose=1, max.K=20, tol.scale.ratio=1e-5){
...
Ls <- scaleL.qp(Y, temp.obj$L, scale.min=0, print.scale=FALSE)

fit.init <- INIT.L(init.L=Ls$Ls, Y=Y)
fit.alt <- alt.flash(fit.init)
plotLF(fit.alt$A.l, fit.alt$A.f, 'nonneg init')
```
...
fit.init <- INIT.L(init.L=Ls$Ls, Y=Y)
fit.alt <- alt.flash(fit.init)
plotLF(fit.alt$A.l, fit.alt$A.f, 'oracle init')
```
...
for (seed in 2:5){
  ...
  plotLF(fit.alt$A.l, fit.alt$A.f, paste0('seed=',seed,': nonneg initialization'))
  
  # oracle init
  fit.alt <- alt.flash(INIT.L(init.L=scaleL.qp(Y, L, scale.min=1e-10, print.scale=FALSE)$Ls, Y=Y))
  plotLF(fit.alt$A.l, fit.alt$A.f, paste0('seed=',seed,': oracle initialization'))
}
