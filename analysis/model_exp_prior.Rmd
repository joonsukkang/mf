...
demo.init.svd <- function(mat.X, r2){
...
demo.update.A <- function(mat.X, sigmasq, delta, Ew.inv,r2){
    ...
    find.k.star <- optimize(f=eval.expectation,
                            interval = c((Y.eigen$values[1]+r2)*sigmasq*(r2+1/delta), (Y.eigen$values[1]+N)*sigmasq*(r2+1/delta))
                            )
...
demo.fit <- function(mat.X, niter, r2){
    ...
    par.init <- demo.init.svd(mat.X, r2) 
...
# X2
mat.X <- X2
obj.fit    <- demo.fit(mat.X, r2=setr2, niter=10) 
