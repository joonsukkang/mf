...
fig.nine.K2 <- function(df, idx.backfit){
    ...
    grid.arrange(fig1, fig2, fig3, fig4, fig5, fig6, fig7, fig8, fig9, nrow=3, ncol=3)
}
```
...
fig.nine.K2(X2n_b, idx.backfit=TRUE)
fig.nine.K2(X2n_b, idx.backfit=FALSE)
```

### data: `X2n_a` (hard case)

```{r, fig.height=6, fig.width=12}
fig.nine.K2(X2n_a, idx.backfit=TRUE)
fig.nine.K2(X2n_a, idx.backfit=FALSE)
```
