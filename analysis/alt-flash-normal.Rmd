...
draw.fig <- function(seed){
  ...
  fit.alt.cf     <- alt.flash(fit.init, n.cores=1) # 'closed-form' method
  ...
  rbind(DF.FIT(fit.f,     'flashier'),
        ...
        DF.FIT(fit.alt.cf,    'alt-flash: closed_form')
  ) -> df.fig
  
  e.tol <- fit.alt.cf$elbo.tol
...
options(cl.cores=set.cores); options(cl.type='FORK')
microbenchmark(flashier=flash.backfit.e(fit.init, verbose.lvl=0),
               flashier.parallel=flash.backfit.e(fit.init, verbose.lvl=0, method='parallel'),
               
               alt.flash=alt.flash(fit.init, n.cores=1, when.normal.closed.form=FALSE),
               alt.flash.parallel=alt.flash(fit.init, n.cores=set.cores, when.normal.closed.form=FALSE),
               alt.flash.closed_form=alt.flash(fit.init, n.cores=1),
               times=50
               ) -> mb
